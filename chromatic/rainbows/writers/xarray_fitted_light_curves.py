"""
    3. Edit the `to_xarray_fitted_light_curves` function so that it will
...
__all__ = ["to_xarray_fitted_light_curves"]
...
required_attrs = [
    ...
    "code",
    "data_origin",
]  # system_params, limb_darkening_params
...
def to_xarray_fitted_light_curves(self, filepath, overwrite=True):
    """
    Write a Rainbow to a file in the xarray_fitted_light_curves format.
    ...
    if k not in self.fluxlike:
        self.fluxlike[k] = self.flux / self.get("systematics_model", 1)

    # kludge to convert to corrected flux
    k = "corrected_flux_error"
    if k not in self.fluxlike:
        self.fluxlike[k] = self.uncertainty / self.get("systematics_model", 1)
