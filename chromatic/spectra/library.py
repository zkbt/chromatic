from ..imports import *
from ..tools.resampling import bintoR, bintogrid
...
class Library:
    """
    Create a library to organize and provide easy access to spectral models.
    """

    # downloaded files will be stored in "~/.{_cache_label}"
    _cache_label = "chromatic"

    # a string name for this library
    _library_name = "?"
    ...
    def _create_upload_command_for_grids(self, destination):
        ...
        """
        source = os.path.join(
            self._directory_for_new_grids, f"{self._library_name}_*.npy"
        )
    ...
    def _get_local_grid(self, R, metallicity=0.0):
        """
        Get the path of a local pre-processed grid,
        either newly created or downloaded to cache.
        """
